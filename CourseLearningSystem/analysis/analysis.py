...
def compute_class_analysis(scores_list: list) -> dict:
    ...
    for s in scores_list:
        ...
        weak_kps.extend(ka.get("weak_points", []))
    unique_weak_kps = list(set(weak_kps))
    ...
    return {
        ...
        "weak_knowledge_points": unique_weak_kps,
        "summary": _generate_summary(avg, avg_rounds, most_common, unique_weak_kps)
    }
...
def _generate_summary(avg, avg_rounds, most_common, weak_kps) -> str:
    ...
    parts.append(f"人均有效交互{avg_rounds}次，常用交互方式为{'、'.join(most_common)}。")
    if weak_kps:
        parts.append(f"全班共性的薄弱知识点集中在：{'、'.join(weak_kps[:3])}等。")
    return " ".join(parts)