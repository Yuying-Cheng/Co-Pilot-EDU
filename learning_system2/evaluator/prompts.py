...
不要直接给总分，只识别证据和等级。
必须严格对照“任务信息”中的 output_requirements/specific_output：
- 如果最终报告没有提交，clarity_score 必须为 0，is_missing_report 为 true。
- 如果报告内容没有覆盖任务指定成果（如思维导图、比对遗漏补全说明、递推式推导、复杂度分析、自设计判断题等），必须在 output_compliance.missing_outputs 中列出。
- 不能因为对话记录内容丰富，就替代最终成果报告和学习反思。
...
{{
  ...
  "clarity": {{
    ...
    "is_missing_report": false
  }},
  "output_compliance": {{
    "matches_task": true,
    "matched_outputs": ["已满足的任务成果点"],
    "missing_outputs": ["缺失或不充分的任务成果点"],
    "reason": "是否符合任务指定提交成果的简短依据"
  }},
  "reflection": {{
    "reflection_score": 6,
    "level": "一般",
