...
from ui.widgets import Worker, SectionHeader, ScoreBar, LoadingWidget, make_info_row, HDivider
                ...
                try:
                    with open(os.path.join(tasks_dir, filename), "r", encoding="utf-8") as f:
                        ...
                        for t in tf.get("tasks", []):
                            ...
                            self._task_combo.addItem(f"{tf.get('chapter_title', chapter_id)} · {t.get('title', '')}", t)
                except Exception:
                    continue
    ...
    def _evaluate(self):
        ...
        for line in text.split('\n'):
            ...
            if line.startswith('学生：') or line.startswith('学生:'):
                current_round['student_input'] = line[3:].strip()
            elif line.startswith('模型：') or line.startswith('模型:'):
                current_round['model_output'] = line[3:].strip()
                if 'student_input' in current_round:
                    dialogues.append(current_round)
                    current_round = {}
        ...
        student_id = self._student_id.text().strip() or "未知学号"
        student_name = self._student_name.text().strip() or "未知姓名"
        task = self._task_combo.currentData()
        chapter_id = task.get("chapter_id", "ch01") if task else "ch01"  # 需要有个默认或真实的章节ID
        ...
        self._loading.set_text(f"正在调用 AI 评阅 {student_name} 的成果…")
        self._loading.show()
        self._loading.raise_()
        self._loading.resize(self.size())

        def do_eval():
            save_report(student_id, chapter_id, self._submission)
            knowledge_dict = load_knowledge(chapter_id)
            task_dict = load_task(chapter_id)
            if not knowledge_dict or not task_dict:
                raise ValueError("缺失该章节的知识点或任务数据，无法打分！")

            score_result = evaluate_submission_v2(
                knowledge=knowledge_dict, task_data=task_dict,
                submission=self._submission, use_llm=True
            )
