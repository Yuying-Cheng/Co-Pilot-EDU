...
def list_courses() -> List[dict]:
    ...
    for fn in sorted(os.listdir(COURSES_DIR)):
        if fn.endswith("_knowledge.json"):
            ...
            courses.append(data)
    # Newest first so task editor auto-selects the most recently created chapter
...
def find_score(student_id: str, chapter_id: str) -> Optional[dict]:
    """查找指定学生+章节的已有评分记录（返回最新一条，无则返回 None）。"""
    found = []
    for fn in os.listdir(SCORES_DIR):
        if not fn.endswith("_score.json"):
            continue
        rec = _load_json(os.path.join(SCORES_DIR, fn))
        if (rec.get("student_id") == student_id and
                rec.get("chapter_id") == chapter_id):
            found.append(rec)
    if not found:
        return None
    return max(found, key=lambda r: r.get("scored_at", ""))
