...
def evaluate_submission(submission: dict, task: dict = None, knowledge: dict = None) -> dict:
    """Full LLM-based evaluation of a student submission."""
...
def parse_dialogue_text(raw_text: str) -> list:
    ...
    """
    dialogues = []
    # Try to split by round markers
    patterns = [
        r'第(\d+)轮[：:\s]*(.*?)(?=第\d+轮|$)',
        r'Round\s*(\d+)[：:\s]*(.*?)(?=Round\s*\d+|$)',
    ]
    ...
    for line in lines:
        ...
        if is_student:
            ...
            if current_student and current_model:
                ...
                current_model = []
            elif current_student and not current_model:
                pass  # student asked twice, skip
            current_student = []
    ...
    # If parsing failed, treat each paragraph as a round
    if not dialogues:
