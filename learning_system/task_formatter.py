...
def format_task_sheet(task_data: Dict[str, Any]) -> str:
    ...
    default_rounds = max(
        [int(t.get("interaction_requirements", {}).get("min_rounds", 10) or 10) for t in tasks] or [10]
    )
    ...
    if task_instruction:
        ...
        lines.append(
            ...
            "交互过程中必须至少包含三种不同的交互方式（如：询问、表达见解、审辨、猜想、想象、创新、"
            "回答苏格拉底式提问等）。提交完整对话记录，并根据任务要求提交相应成果，附个人学习心得。"
        )
    lines.append("")
    ...
    for index, task in enumerate(tasks, start=1):
        ...
        if hints:
            if isinstance(hints, list):
                lines.append("提示：" + "；".join(str(h) for h in hints))
            else:
                lines.append(f"提示：{hints}")

        lines.append(
            f"交互方式要求：至少包含{_join(req.get('required_types', []))}等方式；"
            f"不少于{req.get('min_rounds', default_rounds)}轮。"
        )

        # 优先显示 specific_output
        specific = out.get("specific_output", "").strip()
        if not specific:
            required_outputs = out.get("required_outputs", [])
            specific = "、".join(required_outputs) if required_outputs else "探究报告、交互记录、学习反思"
        word_limit = out.get("word_limit", "500-800字")
        lines.append(f"成果要求：提交{specific}；建议字数{word_limit}。")
        lines.append("")
...
def export_task_sheet_docx(task_data: Dict[str, Any], path: str) -> None:
    """导出任务书为 DOCX 文件。优先使用 python-docx，降级到纯 zip 方式。"""
    try:
        from docx import Document
        from docx.shared import Pt
        _export_with_python_docx(task_data, path)
    except ImportError:
        _export_minimal_docx(format_task_sheet(task_data), path)


def _export_with_python_docx(task_data: Dict[str, Any], path: str) -> None:
    from docx import Document
    from docx.shared import Pt
    ...
    for block in format_task_sheet(task_data).split("\n"):
        if not block.strip():
            doc.add_paragraph("")
        elif "课堂探究任务" in block and block.endswith("课堂探究任务"):
            p = doc.add_paragraph()
            run = p.add_run(block)
            run.bold = True
            run.font.size = Pt(18)
        elif block.startswith("任务") and "：" in block and len(block) < 60:
            p = doc.add_paragraph()
            run = p.add_run(block)
            run.bold = True
            run.font.size = Pt(13)
